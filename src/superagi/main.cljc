(ns superagi.main
  "Kotodama WASM entrypoint for the Superagi clean-room actor (L4) — Clojure port.
  ...
  facts (`superagi.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "superagi")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"superagi.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "superagi-compat" :tier tier :entities entities} 200])
...
(defn start! [] :superagi-compat/ready)
